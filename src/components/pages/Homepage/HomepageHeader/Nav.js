            ...
            <div className={`container-fluid ${styles.navContainer}`}>
                ...
                <div className={`collapse navbar-collapse ${isCollapsed ? '' : 'show'}`} id="navbarSupportedContents">
                    <ul className="navbar-nav ms-auto mb-2 mb-lg-0 justify-content-between align-items-center w-80">
                        <li className={`nav-item first-item text-center ${styles.navLink} bg-white`}><a className="nav-link" href="/discovery">{language === 'en' ? 'DISCOVERY' : '发现'}</a></li>
                        <li className={`nav-item text-center ${styles.navLink} bg-white`}><a className="nav-link" href="/academics">{language === 'en' ? 'ACADEMICS' : '学术'}</a></li>   
                        <li className={`nav-item text-center ${styles.navLink} bg-white`}><a className="nav-link" href="/admission">{language === 'en' ? 'ADMISSION' : '入学'}</a></li>
                        <li className={`nav-item last-item text-center ${styles.navLink} bg-white`}><a className="nav-link" href="/support">{language === 'en' ? 'STUDENT SUPPORT' : '学生支持'}</a></li>
                    </ul>
                </div>
            </div>
        </nav>
