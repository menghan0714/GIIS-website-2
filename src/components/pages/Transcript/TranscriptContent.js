...
function TranscriptContent({ language }) {
  ...
  const calculateOverallGPA = () => {
    ...
    const totalWeightedGPA = semesterGPAs.reduce((sum, gpa) => sum + parseFloat(gpa.weightedGPA || 0), 0);
    const totalUnweightedGPA = semesterGPAs.reduce((sum, gpa) => sum + parseFloat(gpa.unweightedGPA || 0), 0);
...
你的程式碼已經非常完善，只需要稍作調整即可讓子組件計算 Semester Totals 的 GPA 並將其傳遞給父組件。我會基於你目前的程式碼，新增以下內容：

在 GradeTableG9FS 中，傳遞計算完成的 Semester Totals (weighted GPA 和 unweighted GPA) 給父組件 (onGPAUpdate)。
父組件接收來自子組件的 GPA，計算整體 GPA。
以下是修改後的完整實現：

子組件：GradeTableG9FS
在子組件內，當 Semester Totals 更新時，透過 onGPAUpdate 將 weightedGPA 和 unweightedGPA 傳遞給父組件。
...
父組件：TranscriptContent
父組件收集所有子組件的 GPA，並計算整體 GPA。

jsx
複製程式碼
import React, { useState } from "react";
import GradeTableG9FS from "./GradeTableG9FS";
